...
(quais_bissextos <- which(bissextos == 1))
anos[bissextos]
```
...
ndias <- ifelse(
  test = (div_anos_400 == 0) | (div_anos_4 == 0) & (div_anos_100 > 0),yes = 366,
  no = 365
)
names(ndias) <- anos
ndias
...
tmed_estcoes <- c(tmed_est1, tmed_est2)
estacoes <- c("Estação_1", "Estação_2")
names(tmed_estcoes) <- estacoes
tmed_estcoes
```
b.
```{r}
...
f.
```{r}
difh_tar <- c(tar_est2 - tar_est1)
difh_tar[which.max(difh_tar)]
```
g.
```{r}
(h_difh_tar <- c(Hora[which.max(difh_tar)]))
```
h.
```{r}
(difh_tar1 <- c(diff(tar_est1)))
(h_ps_tar1 <- c(which.min(difh_tar1)))
(difh_tar2 <- c(diff(tar_est2)))
(h_ps_tar2 <- c(which.min(difh_tar2)))
```
i.
```{r}
(temp_prox <- c(Hora[which((tar_est1 - tar_est2) < 0.5)]))
```
j.
...
(tar_met2 <- ((tar_est2[8]) + (tar_est2[15]) + (2 * (tar_est2[22]))) / 4)
```
...
arg1 <- ifelse(test = (Hora < hmin_est1),
                      ...
                      no = arg2)
t_est1 <- ifelse(test =  (hmin_est1 <= Hora & Hora < hmax_est1),
                      yes = t_est1 <- tmed_est1 - a1 * cos(arg3),
                      no = t_est1 <- tmed_est1 + a1 * cos(arg3)
)
t_est1
...
arg1 <- ifelse(test = (Hora < hmin_est2),
                      ...
                      no = arg2)
t_est2 <- ifelse(test =  (hmin_est2 <= Hora & Hora < hmax_est2),
                      yes = t_est2 <- tmed - a2 * cos(arg3),
                      no = t_est2 <- tmed + a2 * cos(arg3)
)
t_est2


(RMSE1 <- sqrt(mean((t_est1 - tar_est1)^2)))
(RMSE2 <- sqrt(mean((t_est2 - tar_est2)^2)))


r1 <- (mean((t_est1 - mean(t_est1)) * (t_est1 - mean(t_est1)))) / sqrt(mean((t_est1 - mean(t_est1)^2) * (t_est1 - mean(t_est1)^2)))
r2 <- (mean((t_est2 - mean(t_est2)) * (t_est2 - mean(t_est2)))) / sqrt(mean((t_est2 - mean(t_est2)^2) * (t_est2 - mean(t_est2)^2)))
r1
r2
```
